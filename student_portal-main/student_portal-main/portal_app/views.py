    ...
    if request.method == 'POST' and form.is_valid():
        ...
        try:
            if ext == '.csv':
                ...
                return render(request, 'portal_app/bulk_upload_students.html', {'form': form})

            required_cols = {'student_id', 'name', 'department', 'year', 'email', 'phone'}
            if not required_cols.issubset(df.columns):
                messages.error(request, "Missing required columns: " + ", ".join(required_cols - set(df.columns)))
            else:
                count = 0
